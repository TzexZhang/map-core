...
├── scripts/
│   └── publish.mjs         # 一键发布脚本（Node.js，跨平台）
├── .eslintrc.cjs
├── tsconfig.base.json
├── tsconfig.json
├── turbo.json
...
| ------------------------- | ---------------------------------------- | ---- | ------------------- | ------------------------------------------------------- |
| `@geomapcore/core`           | 类型定义、接口契约、事件系统、工具函数、部署配置                 | ✅    | 无（零依赖）              | 需要 `IDataSource`/`IDataSourceManager` 等 SDK 未导出的接口时单独引用 |
| `@geomapcore/adapter-ol`     | OpenLayers 2D 引擎适配器                      | ❌    | core, ol (peer)     | 不建议直接引用，由 `MapController` 内部创建                          |
| `@geomapcore/adapter-cesium` | Cesium 3D 引擎适配器                          | ❌    | core, cesium (peer) | 不建议直接引用，由 `MapController` 内部创建                          |
| `@geomapcore/datasource`     | HTTP/WebSocket 数据源管理、中间件                 | ❌    | core                | 不建议直接引用，SDK 内部私有能力，外部通过 `ICustomDataSource` 注入数据        |
| `@geomapcore/bridge`         | 跨端通信桥（Android/iOS/Qt）                    | ✅    | core                | 已通过 SDK 导出 `BridgeFactory`/`BridgeEnvironment`，通常无需单独引用 |
| `@geomapcore/sdk`            | 聚合包，统一导出入口；提供 `mapEngineSetup()` Vite 插件 | ✅    | 所有子包                | **始终安装**，所有项目的唯一入口                                      |
...
{
  "dependencies": {
    "@geomapcore/sdk": "file:../MapCore/packages/sdk"
  }
}
```
...
| 2. Cesium 资源 | `mapEngineSetup()` 插件自动处理（开发 + 生产构建），无需手动部署     |
| 3. 环境变量      | 配置 `.env.production` 指向内网服务地址                   |
| 4. 禁用 Ion    | 设置 `MAPCORE_CESIUM_ION=null` 禁用 Cesium Ion 外网请求 |
...
1. 在 [npmjs.com](https://www.npmjs.com/) 注册账号并登录
2. 创建 `geomapcore` 组织（头像 → Add Organization → 输入 `geomapcore` → 选择 Unlimited public packages）
3. 在终端登录 npm：
...
> **国内用户**：使用淘宝镜像时，登录和发布需指定官方源（不影响全局配置）。

### 一键发布

```bash
# 设置 Token（每次终端会话只需设置一次）
# 在 npmjs.com → 头像 → Access Tokens → Generate New Token (Granular) 获取
set NPM_TOKEN=npm_xxxxxxxxxxxxxxxx    # Windows CMD
$env:NPM_TOKEN="npm_xxxxxxxxxxxxxxxx" # PowerShell

# 指定版本号发布
pnpm publish:sdk <version>

# 示例
pnpm publish:sdk 1.0.1    # PATCH - Bug 修复
pnpm publish:sdk 1.1.0    # MINOR - 新功能
pnpm publish:sdk 2.0.0    # MAJOR - 破坏性变更
```

脚本会自动完成：更新所有子包版本号 → 更新 lockfile → 构建 → 验证产物 → 按依赖顺序发布。

> **Token 安全**：Token 仅通过环境变量 `NPM_TOKEN` 传入，不写入全局配置文件。关闭终端后 Token 自动失效。

### Dry Run（预演）

```bash
pnpm publish:sdk:dry
```

仅打印将要执行的命令，不实际发布。用于发布前检查。
...
```bash
cd packages/core && npm publish --registry=https://registry.npmjs.org
cd packages/datasource && npm publish --registry=https://registry.npmjs.org
cd packages/bridge && npm publish --registry=https://registry.npmjs.org
cd packages/adapter-ol && npm publish --registry=https://registry.npmjs.org
cd packages/adapter-cesium && npm publish --registry=https://registry.npmjs.org
cd packages/sdk && npm publish --registry=https://registry.npmjs.org
```
...
```bash
npm view @geomapcore/sdk versions
npm view @geomapcore/core versions
```

### 使用发布的包

```bash
pnpm add @geomapcore/sdk ol cesium
```
