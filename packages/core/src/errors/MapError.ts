...
export class MapError extends Error {
  ...
  public readonly errorCode: MapErrorCode;
  /** 错误发生的模块名称 */
   ...
   * @param cause - 原始错误（可选）
   */
  constructor(message: string, errorCode: MapErrorCode, module: string = 'unknown', cause?: Error) {
    super(message);
    this.name = 'MapError';
  ...
  static engineNotReady(method: string): MapError {
    return new MapError(
      `引擎未初始化，无法调用 ${method}()`,
      MapErrorCode.E2001_ENGINE_NOT_READY,
      'Engine'
    );
  }
  ...
  static layerNotFound(layerId: string): MapError {
    return new MapError(
      `图层 "${layerId}" 不存在`,
      MapErrorCode.E3002_LAYER_NOT_FOUND,
      'LayerManager'
    );
  }
  ...
  static sourceNotFound(sourceId: string): MapError {
    return new MapError(
      `数据源 "${sourceId}" 不存在`,
      MapErrorCode.E4002_SOURCE_NOT_FOUND,
      'DataSourceManager'
    );
  }
  ...
  static fetchFailed(sourceId: string, cause?: Error): MapError {
    return new MapError(
      `数据源 "${sourceId}" 请求失败`,
      MapErrorCode.E4003_SOURCE_FETCH_FAILED,
      'DataSource',
      cause
    );
  }
