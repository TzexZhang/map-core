 ...
 */
export type DataSourceConfig = HttpSourceConfig | WebSocketSourceConfig | StaticSourceConfig | MockSourceConfig;
/**
 * 数据源中间件接口
...
export interface DataSourceGlobalConfig {
    headers?: Record<string, string>;
    timeout?: number;
    requestInterceptor?: (url: string, init: RequestInit) => Promise<{
        url: string;
        init: RequestInit;
    }>;
