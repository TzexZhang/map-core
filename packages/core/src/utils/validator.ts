...
export function assertNumberInRange(
  ...
  if (typeof value !== 'number' || !Number.isFinite(value) || value < min || value > max) {
    throw new Error(
      `参数校验失败: ${paramName} 必须是 ${min}~${max} 之间的有效数字，实际值: ${value}`
    );
  }
}
 ...
 */
export function assertDefined(value: unknown, paramName: string): void {
 ...
 */
export function assertUniqueId(id: string, existingIds: Set<string>, resourceType: string): void {
 ...
 */
export function assertValidLngLat(lngLat: unknown, paramName: string): void {
  if (
    ...
    throw new Error(
      `参数校验失败: ${paramName} 必须是有效的经纬度坐标 [经度(-180~180), 纬度(-90~90)]`
    );
  }
}
...
export function assertEnumValue(
  ...
  if (!validValues.includes(value as string)) {
    throw new Error(
      `参数校验失败: ${paramName} 必须是 [${validValues.join(', ')}] 之一，实际值: ${value}`
    );
  }
}
