...
<script type="module">
  import 'https://cdn.jsdelivr.net/npm/@mapcore/sdk/dist/webcomponent.js';
  const mapEl = document.querySelector('map-core');
  mapEl.addEventListener('map-ready', (e) => {
    e.detail.controller.addLayer({ id: 'targets', type: 'vector' });
  });
</script>
```
...
| 属性        | 说明                           | 示例                        |
| --------- | ---------------------------- | ------------------------- |
| `engine`  | 引擎类型：`openlayers` / `cesium` | `engine="openlayers"`     |
| `center`  | 初始中心点（经度,纬度）                 | `center="116.397,39.909"` |
| `zoom`    | 初始缩放级别                       | `zoom="10"`               |
| `pitch`   | 俯仰角（仅 3D）                    | `pitch="-45"`             |
| `heading` | 朝向角（仅 3D）                    | `heading="0"`             |
...
**Vite 配置**（`vite.config.ts`）— Cesium 需要静态资源
...
> **插件安装**：`pnpm install -D vite-plugin-static-copy`
...
| **静态资源** | Cesium Workers/Assets/Widgets 需复制到可访问路径（通过 Vite 插件或手动） |
...
<map-core engine="cesium" center="116.397,39.909" zoom="10" pitch="-45"></map-core>

<script type="module">
  import 'https://cdn.jsdelivr.net/npm/@mapcore/sdk/dist/webcomponent.js';
  const mapEl = document.querySelector('map-core');
  mapEl.addEventListener('map-ready', (e) => {
    e.detail.controller.addLayer({ id: 'buildings', type: 'tileset3d', url: '/3dtiles/tileset.json' });
  });
</script>
```
...
| **3D (Cesium)**     | OSM 影像 + Cesium World Terrain |
...
| `basemap`        | `BasemapConfig`   | ❌  | 底图配置。不传使用默认底图（2D: OSM，3D: OSM + World Terrain）。传 `{ preset: 'blank' }` 不加载底图                                                                                          |
...
| `LayerType.Terrain`   | 地形服务          | Cesium      | `url`                       |
...
| 项目              | 说明                                                                                                                   |
| --------------- | -------------------------------------------------------------------------------------------------------------------- |
| **CSS（自动注入）**   | SDK 在引擎初始化时自动注入 `ol/ol.css`（2D）或 `cesium/widgets.css`（3D）。Vite/Webpack 5 项目无需手动引入；纯 HTML/离线环境 SDK 自动回退为 CDN `<link>` |
| **容器尺寸**        | 地图容器 DOM **必须有明确的宽高**（如 `height: 600px`），`height: 0` 或 `display: none` 会导致不渲染                                        |
| **底图**          | 默认自动加载（2D: OSM，3D: OSM + World Terrain）。可通过 `basemap` 选项自定义                                                          |
| **坐标系**         | 默认 EPSG:4326（经纬度），可通过 `coordinateSystem: 'EPSG:3857'` 切换为投影坐标（米）。GeoJSON 数据始终使用 EPSG:4326                      |
| **Cesium 静态资源** | 3D 模式需要将 Workers/Assets/Widgets 复制到可访问路径，并配置 `CESIUM_BASE_URL`                                                       |
| **TypeScript**  | SDK 包含完整类型定义（`.d.ts`），无需额外安装 `@types/` 包                                                                             |
| **销毁**          | 页面/组件卸载时必须调用 `map.destroy()` 释放资源                                                                                    |
